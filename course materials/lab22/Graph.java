...
public class Graph implements Iterable<Integer> {
    ...
    public List<Integer> shortestPath(int start, int stop) {
        ...
        ArrayList<Vertex> pathTable = new ArrayList<>();
        // visited.add(start);
        Vertex add;
        // for (int i = 0; i < vertexCount; i++) {
        //     if (i == start) {
        //         add = new Vertex(start, start, 0);
        //     }else {
        //         add = new Vertex(i, 10000,10000);
        //     }
        //     pathTable.add(i, add);
        // }
        add = new Vertex(start, start, 0);
        ...
        while (!fringe.isEmpty()) {
            ...
            if (!pathTable.contains(v)) {
                pathTable.add(v);
                if(v.to() == stop){
                    break;
                }
                for (int n: neighbors(v.to())) {
                    // if (!visited.contains(n)) {

                    add = new Vertex(v.to(), n, getEdge(v.to(), n).weight()+ v.distance());
                    fringe.add(add);
        ...
        return restorePath(pathTable, start, stop);
    }




    private ArrayList<Integer> restorePath(ArrayList<Vertex> pathTable, int start, int stop) {
        ArrayList<Integer> path = new ArrayList<>();
        int pos = stop;

        while (pos != start) {

            for (Vertex v: pathTable) {
                if(v.to() == pos) {
                    path.add(0, v.to());
                    pos = v.from();
                    break;
    ...
    private class Vertex implements Comparable<Vertex>{
        ...
        public int to(){return desPos;}
        public int from(){return selfPos;}

        public boolean equals(Vertex v, Vertex other) {
        ...
        public void update(Vertex origin, int newWeight){
            origin.setDistance(newWeight);
        }
