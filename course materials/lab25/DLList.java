...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        // TODO: YOUR CODE HERE
        this.addLast(item);
        for (Node pre = this.sentinel.prev.prev; pre != this.sentinel; pre = pre.prev) {
            if (item.compareTo(pre.item) < 0) {
                Node tmp = pre.next.next;
    ...
    // private DLList<T> mergehelper(DLList<T> lst) {
    //     if (size <= 1) {
    //         return this;
    //     }
    ...
    public DLList<T> quicksort() {
        ...
        DLList<T> k = smallElements.quicksort();
        k.append(equalElements);
        k.append(largeElements.quicksort());
