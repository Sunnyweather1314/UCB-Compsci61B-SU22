...
//            s.createNewFile();
    ...
    public static void writeStory(String[] args) {
        ...
        File store = new File("story.txt");

        String content = Utils.readContentsAsString(store);
        if(content.length() == 0){
            Utils.writeContents(store , args[1]);
        }else{
            content = content +"\n"+ args[1];
            Utils.writeContents(store , content);
        }
        System.out.println(Utils.readContentsAsString(store));
        // FIXME
    }
    ...
    public static void makeDog(String[] args) {
        ...
        if(Integer.parseInt(args[3]) <0){
            exitWithError("Wrong number of inputs");
        }
