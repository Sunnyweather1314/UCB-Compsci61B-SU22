...
public class Graph {
    ...
    public Graph prims(int start) {
        ...
        ArrayList<Edge> pathTable = new ArrayList<>();
        ...
        while (!fringe.isEmpty()) {
            Edge v = fringe.poll();
            if (!pathContains(pathTable, v.getDest())) {
                pathTable.add(v);
                MST.addEdge(v);

                for (Edge n: getEdges(v.getDest())) {
    ...
    // private boolean update(Edge a, Edge b) {
    //     if (a.equals(b)) {
    //         return false;
    //     } else if (a.getDest() == b.getDest() && a.getWeight() > b.getWeight()) {
    //         return true;
    //     } else {
    //         return false;
    //     }
    ...
    private boolean pathContains(ArrayList<Edge> pathtable, int i) {
        for (Edge e: pathtable) {
            if (e.getDest() == i) {
                return true;
            }
        }
    ...
    public Graph kruskals() {
        ...
        UnionFind unionVertex = new UnionFind(getAllVertices().size());
//
...
//        }
        while (MST.getAllVertices().size() < getAllVertices().size()) {
            Edge add = (Edge)iter.next();
            if (!unionVertex.connected(add.getSource(), add.getDest())) {
                MST.addEdge(add);
                unionVertex.union(add.getSource(), add.getDest());
            }
        }
